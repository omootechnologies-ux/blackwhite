...
const navItems = [
  ...
  { href: '/dashboard/tax', labelKey: 'common.taxCompliance', icon: 'TRA' },
...
export function SidebarNav() {
  ...
  return (
    <nav className="flex gap-2 overflow-x-auto px-3 py-3 lg:block lg:flex-1 lg:space-y-0.5 lg:overflow-visible lg:py-4">
      {navItems.map((item) => {
        ...
        return (
          <Link
            key={item.href}
            href={item.href}
            className={`flex shrink-0 items-center gap-2.5 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${
              active
